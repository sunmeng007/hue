  ...
  def decorator(*args, **kwargs):
    ...
    while retries < 15:
      try:
        ...
        if not is_ha_enabled() or retries == 15:
          ...
          time.sleep(1)
          args[0].client = get_cached_client(args[0].client.username, retries=retries)          
          LOG.info('Picked %s at try %s' % (args[0].client, retries))
          retries += 1
...
def get_cached_client(username, component=None, retries=0):
  exempt_host = None
  ...
  if API_CACHE is None:
    ...
    try:
      API_CACHE = get_sentry_client(username, SentryClient, exempt_host=exempt_host, component=component, retries=retries)
